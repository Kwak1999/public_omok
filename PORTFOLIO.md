...
### **실시간 멀티플레이어 (Socket.IO + Room)**
- **Room 기반 매칭**: 방 생성/참가 시 Room에 연결해 게임 상태를 격리합니다.
- **실시간 동기화**: 착수, 승패, 차례 전환을 이벤트로 브로드캐스트합니다.
- **재연결/정리 로직**: 연결 해제 시 방 정리, 재연결 시 상태 복원을 지원합니다.

---

### **렌주룰 엔진 (3-3 / 4-4 / 6목 금지)**
- **서버 검증**: 클라이언트 치팅을 방지하기 위해 서버가 금수 규칙을 판단합니다.
- **8방향 패턴 탐색**: 연속 패턴과 갭 패턴을 동시에 탐색해 금수 여부를 계산합니다.
- **승리 판정**: 5목 완성 시 승리, 장목(6목 이상)은 금지 규칙으로 차단합니다.

---

### **타이머 시스템 (메인 + 초읽기)**
- **메인 타이머 40초**와 **턴 타이머 20초**를 분리 관리합니다.
- **시간 초과 처리**: 제한 시간 종료 시 서버가 차례를 전환합니다.
- **실시간 게이지**: 남은 시간을 UI로 시각화해 플레이어가 즉시 인지할 수 있습니다.

---

### **공개방/게스트 인증**
- **공개방 목록 조회**: 공개방 정보를 API로 제공하고 UI에서 즉시 갱신합니다.
- **게스트 ID 발급**: 별도 회원가입 없이 게스트 ID로 플레이를 시작합니다.
- **24시간 유효 세션**: 브라우저 재접속 시에도 동일 게스트로 기록을 이어갑니다.

---

### **경기 기록 & 복기**
- **경기 기록 저장**: 게임 종료 시 착수 기록과 승패 정보를 저장합니다.
- **게스트별 기록 조회**: 게스트 ID 기준으로 경기 기록을 조회합니다.
- **복기 재생**: 저장된 착수 순서를 기반으로 리플레이 화면을 제공합니다.
...
### **S — Situation (상황)**
실시간 통신이 필요한 오목 서비스를 구축하면서,  
렌주룰 검증·타이머·경기 기록까지 포함한 **게임 플랫폼**을 완성해야 했습니다.

---

### **T — Task (과제)**
1. **Socket.IO 기반 멀티플레이어 구조 안정화**
2. **렌주룰 금수 규칙을 서버에서 정확히 검증**
3. **경기 기록/복기를 포함한 사용자 경험 완성**

---

### **A — Action (행동)**
#### 1️⃣ 실시간 게임 흐름을 Room 아키텍처로 표준화
- 방 생성/참가/나가기 이벤트를 분리하고,  
  상태 변경을 서버 중심으로 동기화했습니다.

#### 2️⃣ 렌주룰 패턴 탐색을 서버 검증으로 통일
- 8방향 패턴 탐색과 갭 패턴을 동시에 체크하여  
  3-3/4-4/6목 금지 규칙을 일관되게 처리했습니다.

#### 3️⃣ 경기 기록 저장과 복기 UI를 분리 설계
- 게임 종료 시 기록을 저장하고,  
  리플레이 화면에서 착수 순서를 재현하도록 구성했습니다.

---

### **R — Result (결과)**
- 실시간 멀티플레이어와 규칙 검증이 충돌 없이 동작
- 서버 중심 데이터 흐름으로 **안정적인 게임 진행** 확보
- 기록/복기까지 포함한 **완성형 게임 서비스** 구현

이 프로젝트를 통해  
**실시간 게임 서비스의 아키텍처와 흐름 설계 역량**을 강화했습니다.
